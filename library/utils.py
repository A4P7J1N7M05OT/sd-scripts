...
def vips_resize(image, size):
    ...
    vips_image = vips_image.resize(
        target_width / width, vscale=target_height / height, kernel=pyvips.enums.Kernel.MKS2021
    )

    resized_np = np.ndarray(
        buffer=vips_image.write_to_memory(), dtype=np.uint8, shape=(vips_image.height, vips_image.width, bands)
    )
...
def resize_image(
    ...
    if use_pil:
        ...
        logger.debug(f"resize image using {resize_interpolation} (cv2 multistep)")
